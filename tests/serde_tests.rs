use vhdl_doc::memory_map::schema::MemoryMap;
use std::fs;
use toml;

#[test]
pub fn toml_to_json() {
    let contents = fs::read_to_string("tests/assets/memory_map.toml").expect("Failed to read file");
    let memory_map : MemoryMap = toml::from_str(&contents).expect("Failed to parse TOML");
    println!("{}", serde_json::to_string_pretty(&memory_map).expect("Failed to serialize to JSON string"));
}

#[test]
pub fn json_to_toml() {
    let contents = fs::read_to_string("tests/assets/memory_map.json").expect("Failed to read file");
    let memory_map : MemoryMap = toml::from_str(&contents).expect("Failed to parse JSON");
    println!("{}", serde_json::to_string_pretty(&memory_map).expect("Failed to serialize to TOML string"));
}
